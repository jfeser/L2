...
let rec enumerate
  ...
  in
  let op_matrix op op_typ =
    callable_matrix (Expr.Op.cost op) (fun ret_typ args -> TypedExpr.Op ((op, args), ret_typ)) op_typ
  in
  let apply_matrix func func_typ =
    callable_matrix 1 (fun ret_typ args -> TypedExpr.Apply ((func, args), ret_typ)) func_typ
  in
...
let solve_single
  ...
  let matrix_of_hole hole =
    let init' =
      (Ctx.to_alist hole.tctx |> List.map ~f:(fun (name, typ) -> TypedExpr.Id (name, typ))) @ init
    in
    match hole.signature with
    | Arrow_t (arg_typs, ret_typ) ->
      let args = List.map arg_typs ~f:(fun typ -> Fresh.name "x", typ) in
      let arg_names, _ = List.unzip args in
      let init'' = init' @ (List.map args ~f:(fun (name, typ) -> TypedExpr.Id (name, typ))) in
      if config.untyped then
        ...
        |> Sstream.map_matrix ~f:(fun texpr -> `Lambda (arg_names, TypedExpr.to_expr texpr))
    | typ ->
      if config.untyped then
        simple_enumerate init'
      else
        enumerate config init' typ |> Sstream.map_matrix ~f:TypedExpr.to_expr
  in
  ...
  let search max_cost spec : (expr -> expr) option =
    ...
    let rec search' (exh_cost: int) : (expr -> expr) option =
      if (total_cost spec.Structure.Spec.cost exh_cost) >= max_cost then begin
        (if exh_cost > 0 then
...
let solve ?(config=default_config) ?(bk=[]) ?(init=default_init) examples =
  ...
  in
  let vctx =
    List.fold_left bk ~init:default_stdlib
      ~f:(fun ctx (name, impl) ->
          Ctx.bind ctx name (`Closure (impl, ctx)))
  in

  let init =
    init @ (Ctx.to_alist tctx |> List.map ~f:(fun (name, typ) -> TypedExpr.Id (name, typ)))
  in
