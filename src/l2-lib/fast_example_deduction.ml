...
module Abstract_list = struct
    ...
    fun { max_length } values ->
      ...
      let xs = 
        ...
        |> Seq.concat_map ~f:(fun len ->
            Combinat.combinations_with_replacement len values
            |> Seq.of_list)
  ...
  let lift : domain -> 'a list -> 'a value =
    fun { max_length } l ->
      if List.length l > max_length then Omega else List l
end
...
type domain = {
  int : Abstract_int.domain;
  list : Abstract_list.domain;
  eq : Abstract_eq.domain;
} [@@deriving sexp]
...
module Abstract_value = struct
  ...
  let rec list_equal =
    fun ~equal:eq x y -> match x, y with
      | [], [] -> `Bool true
      | x::xs, y::ys ->
        begin match eq x y with
          | `Top -> `Top
  ...
  let rec equal =
    fun x y -> match x, y with
      ...
      | `List y, `AbsList (Abstract_list.List x) -> list_equal ~equal x y
  ...
  let rec join =
    fun x y -> match x, y with
      ...
      | `AbsList (Abstract_list.List x), `List y
      | `List y, `AbsList (Abstract_list.List x) -> list_join ~join x y
      | `Bool x, `Bool y -> if x = y then `Bool x else `Top
      | `List x, `List y -> list_join ~join x y
      | `Tree _, _ | _, `Tree _ 
      | `Closure _, _ | _, `Closure _ -> failwith "Unsupported"
      | `Bottom, `Bottom -> `Bottom
      | _ -> `Top
  ...
  let rec to_string = function
    ...
    | `AbsList (Abstract_list.List x)
    | `List x -> 
      List.map x ~f:to_string
      |> String.concat ~sep:"; "
      |> sprintf "[%s]"
  ...
  let rec of_value : int_domain:Abstract_int.domain -> Value.t -> t =
    fun ~int_domain -> function
      ...
      | `Closure (expr, ctx) ->
        `Closure (Abstract_expr.of_expr expr,
                  Ctx.map ctx ~f:(of_value ~int_domain))
      | `Unit -> `Bottom
  ...
  let eval :
    ...
    -> Abstract_expr.t
    -> t =
    fun ?recursion_limit:(limit = `Unlimited) ?(ctx = Ctx.empty ()) ~int_domain ~list_domain expr ->
      let rec ev ctx count expr =
        let continue =
          match limit with
          | `Unlimited -> true
          | `Limited lim -> count < lim
        in
        if not continue then begin
          ...
          | `Apply (func, raw_args) ->
            ...
            begin match ev ctx count func with
              | `Closure (`Lambda (arg_names, body), enclosed_ctx) ->
                begin match List.zip arg_names args with
                  | Some bindings ->
                    let ctx' = List.fold_left bindings ~init:enclosed_ctx
          ...
          | `Op (Expr.Op.If, args) ->
            begin match args with
              | [ux; uy; uz] ->
                begin match ev ctx count ux with
          ...
          | `Op (op, args) ->
            ...
            if List.mem ~equal:(=) args `Bottom then `Bottom else begin
              ...
              | O.Eq -> (match args with
                  | [x; y] -> begin match equal x y with
                      | `Bool x -> `Bool x
                      | `Bottom -> `Bottom
                      | `Top -> `Top
                    end
  ...
  let rec lift : ?ctx:int Value.Map.t -> domain -> Type.t -> Value.t -> t * int Value.Map.t =
    ...
    fun ?(ctx = Value.Map.empty) d t v -> 
      ...
      let rec lift d t v =
        match t, v with
        | T.Const_t T.Num_t, `Num x ->
          `AbsInt (Abstract_int.lift0 d.int x)
        ...
        | T.Var_t _, v -> 
          begin match Map.find !ctx v with
        ...
        | t, v -> failwiths "Unsupported or ill-typed." (t, v) [%sexp_of:Type.t * Value.t]
      in
  ...
  let lower : ?ctx:Value.t Int.Map.t -> t -> Value.t =
    fun ?(ctx = Int.Map.empty) v ->
      let rec lower = function
        ...
        | `AbsList (Abstract_list.List x)
        | `List x -> `List (List.map ~f:lower x)
        | `Bool x -> `Bool x
        | `Top -> raise TopValue
        | `Bottom -> raise BottomValue
        | `AbsInt Abstract_int.Omega
        | `AbsEq Abstract_eq.Omega 
        | `AbsList Abstract_list.Omega -> raise TopValue
...
module Abstract_example = struct
  ...
  let lift : domain -> Type.t -> (Value.t list * Value.t) -> (t * Value.t Int.Map.t) =
    ...
    fun domain type_ (ins, out) ->
      ...
      | Arrow_t (ins_t, out_t) -> 
        let ins_rev, ctx = 
          List.fold2_exn ins ins_t ~f:(fun (ins, ctx) v t -> 
            let v', ctx = Abstract_value.lift ~ctx domain t v in
            (v'::ins, ctx)) ~init:([], Value.Map.empty)
        in
  ...
  let lower : Value.t Int.Map.t -> t ->
    [`Ex of [`Value of Value.t | `Top] list * [`Value of Value.t | `Top] | `Bottom] =
    fun ctx (ins, out) ->
      try
        let ins = List.map ins ~f:(fun i ->
            try `Value (Abstract_value.lower ~ctx i)
            with Abstract_value.TopValue -> `Top)
        in
        let out =
          try `Value (Abstract_value.lower ~ctx out)
          with Abstract_value.TopValue -> `Top
        in
        `Ex (ins, out)
      with 
      | Abstract_value.BottomValue -> `Bottom
      ...
      | None ->
        let v =
          Eval.eval
            ...
            expr
        in
        ignore (Hashtbl.add table ~key:(expr, ctx) ~data:v); v
  ...
  let of_spec_and_args :
    ...
    let lift :
      ...
      fun expr_ctx eval_ctx domain type_ ins out ->
        ...
        | Arrow_t (ins_t, out_t) -> 
          let ins_rev, ctx = 
            List.fold2_exn ins ins_t ~f:(fun (ins, ctx) sk t -> 
                let v', ctx =
                  try
                  ...
                  | Eval.HitRecursionLimit ->
                    cincr "recursionlimit_failure"; (`Top, ctx)
                  | Eval.RuntimeError _ ->
                    cincr "eval_failure"; (`Bottom, ctx)
                in
                (v'::ins, ctx)) ~init:([], Value.Map.empty)
          in
...
let infer_examples :
  ...
  fun ~library ~specs ~op ~args exs ->
    ...
    | Some op_specs -> begin
        ...
        in
        let arg_specs =
          List.map2_exn ctxs deduced_exs ~f:(fun ctx ->
              function
