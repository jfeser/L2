...
let generate_expr : cost:int -> Library.t -> Type.t -> Expr.t list =
  ...
  fun ~cost library type_ ->
    ...
    let rec loop hypo =
      ...
      in
      if List.length concrete > 0 then concrete |> List.map ~f:H.to_expr
      else List.concat_map small_abstract ~f:loop
      (* let choices = *)
  ...
  fun ~cost library type_ ->
    ...
    let rec loop hypo =
      ...
      let concrete =
        List.filter hypos ~f:(fun h -> H.cost h = cost && H.kind h = H.Concrete)
      in
      if List.length concrete > 0 then Seq.of_list concrete
      else Seq.concat_map (Seq.of_list small_abstract) ~f:loop
      (* let choices = *)
      (*   (if List.length small_abstract > 0 then [`Abstract] else []) @ *)
      (*   (if List.length concrete > 0 then [`Concrete] else []) *)
      (* in *)
...
let generate_spec : Value.t list list -> Library.t -> Type.t -> Expr.t -> Sp.t =
 fun inputs l t e ->
  try
...
let is_interesting : Sp.t -> bool =
 fun spec ->
  ...
  | FunctionExamples.FunctionExamples exs ->
      let outs = FunctionExamples.to_list exs |> List.map ~f:Tuple.T2.get2 in
