...
let test_eval =
  make_tests ~in_f:(fun str -> str |> Util.parse_expr |> eval) ~out_f:identity
              ...
              [ "1", `Num 1;
                ...
                "(map [0 1 0] (lambda (e:num):bool (= e 0)))",
                `List ([`Bool true; `Bool false; `Bool true], Bool_t);
              ]
...
let test_eval_prog =
  make_tests ~in_f:(fun str -> str |> Util.parse_prog |> Eval.prog_eval) ~out_f:identity
              ...
              [ "(define a 1) (+ a 1)", `Num 2;
                ...
                "(define a 1)", `Unit;
                "1 2 3", `Num 3;
                "(define last (lambda (l:[num]):num (if (= []:num (cdr l)) (car l) (last (cdr l)))))
                 (last [1 2 3])", `Num 3;
                "(define f6 (lambda (e8:num):num (+ 1 e8))) (define f (lambda (x7:[num]):[num] (map x7 f6)))
                 (f [1 2 3])", `List ([`Num 2; `Num 3; `Num 4], Num_t);
                "(define f (lambda (ll:[[num]]):[num] (map ll (lambda (l:[num]):num (fold l (lambda (a:num e:num):num (+ a e)) 0)))))
                 (f [[1 2] [3 4]])", `List ([`Num 3; `Num 7], Num_t);
              ]
