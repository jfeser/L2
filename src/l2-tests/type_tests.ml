...
let unifier_of_types_tests =
  ...
  mk_equality_tests
    ...
    ~cmp:(Option.equal Unifier.equal)
    "unifier-of_types-tests"
    ...
    [
      ...
      (arrow1 (free 0 0) (free 1 0), arrow1 num (free 2 0)), Some (Unifier.of_alist_exn [0, num; 2, free 1 0]);
...
let unifier_tests = "unifier-tests" >::: [
    unifier_of_types_tests;
  ]
...
let infer_typeof_tests =
  ...
  mk_equality_tests
    ...
    ~cmp:Type.equal
    "infer-typeof-tests"
    (fun input -> let t, u = Type.of_expr input in t)
    cases
