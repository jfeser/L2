...
let tree_max_tests =
  ...
  "tree-max-tests" >::: List.map ~f:(fun (input, output) ->
...
let tree_size_tests =
  let module T = Tree in
  "tree-size-tests" >::: List.map ~f:(fun (input, output) ->
...
let tree_map_tests =
  let module T = Tree in
  "tree-map-tests" >::: List.map ~f:(fun (tree, f, output) ->
    test_case (fun _ -> assert_equal (Tree.map tree ~f:f) output)
  ) [
