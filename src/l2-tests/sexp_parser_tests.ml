...
let test_parse_expr =
  ...
  make_tests ~in_f:Expr.of_string_exn ~out_f:ident
    ~in_str:ident ~out_str:Expr.to_string ~res_str:Expr.to_string
    "parse_expr"
    [ "1", `Num 1;
      "#t", `Bool true;
      "#f", `Bool false;
...
let test_parse_typ =
  make_tests ~in_f:Type.of_string_exn ~out_f:ident
    ~in_str:ident ~out_str:Type.to_string ~res_str:Type.to_string
    "parse_typ"
    [ "num", Const_t Num_t;
    ]

let test_parse_example =
  make_tests ~in_f:Example.of_string_exn ~out_f:ident
    ~in_str:ident ~out_str:Example.to_string ~res_str:Example.to_string
    "parse_example"
