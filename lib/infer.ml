...
module Unifier = struct
  ...
  let merge outer inner =
    Int.Map.merge outer inner ~f:(fun ~key:_ ->
      function `Both (_, i) -> Some i | `Left x | `Right x -> Some x)
...
let typeof (ctx : typ Ctx.t) (level : level) (expr : expr) : TypedExpr.t =
  ...
  let rec typeof' (ctx : _ Ctx.t) level expr =
    ...
    | `Apply (func, args) ->
        ...
        | Unequal_lengths -> error (Info.of_string "Wrong number of arguments.") );
...
module Type = struct
  ...
  let generalize ctx t =
    ...
    let rec gen = function
      ...
      | (Const_t _ | Var_t { contents = Quant _ } | Var_t { contents = Free _ }) as t
        ->
          t
    in
    gen t
