...
module SimpleMemoizer =
  Sstream.Memoizer
    (struct
      type t = TypedExpr.t list [@@deriving sexp, compare]
    end)
    (Expr)
...
let rec enumerate ?(ops = default_operators) ?(memo = TypMemoizer.empty ()) config
    init typ (check : TypedExpr.t -> bool) : TypedExpr.t Sstream.matrix =
  let open Sstream in
  ...
  let args_matrix (check : TypedExpr.t list -> bool) (arg_typs : typ list) =
    let dummy_args =
      List.map ~f:(fun t -> TypedExpr.Id (Fresh.name "d", generalize 0 t)) arg_typs
    in
    let choose (prev_args : TypedExpr.t list) : TypedExpr.t list matrix =
      ...
      slazy (fun () ->
          ...
          if should_prune_branch then Sstream.repeat []
          else
            map_matrix (enumerate config ~memo init current_typ' check')
              ~f:(fun arg -> prev_args @ [ arg ]))
    in
    match arg_typs with
    | _ :: xs ->
  ...
  let callable_matrix check cost apply_callable callable_typ =
    ...
    | Arrow_t (arg_typs, ret_typ) ->
        ...
        let matrix =
          slazy (fun () ->
              map_matrix (args_matrix check arg_typs) ~f:(apply_callable ret_typ))
        in
        concat prefix matrix
    | _ -> arrow_error ()
  in
...
let solve_single ?(init = []) ?(verify = Verify.verify_examples ~ctx:Ctx.empty)
  ...
  let generate_specs (specs : Spec.t list) : Spec.t list =
    List.concat_map specs ~f:(fun parent ->
        ...
        @ Spec.fold_bodies ~deduce_examples:config.Config.deduction
            ~infer_base:config.Config.infer_base parent
        @ Spec.foldt_bodies ~deduce_examples:config.Config.deduction
            ~infer_base:config.Config.infer_base parent
  ...
  let matrix_of_hole (check : TypedExpr.t -> bool) (hole : hole) :
      expr Sstream.matrix =
    let init' =
    ...
    | Arrow_t (arg_typs, ret_typ) ->
        ...
        if config.Config.untyped then
          ...
          enumerate config init'' ret_typ (fun e ->
              check (TypedExpr.Lambda ((arg_names, e), hole.signature)))
          |> Sstream.map_matrix ~f:(fun e ->
                 `Lambda (arg_names, TypedExpr.to_expr e))
    | typ ->
        if config.Config.untyped then simple_enumerate init'
        else
          enumerate config init' typ check |> Sstream.map_matrix ~f:TypedExpr.to_expr
  in
  ...
  (* Given a specification, create a stream of options. Any expression that
     verifies correctly will be a Some variant. *)
  let solver_of_spec (spec : Spec.t) : (expr -> expr) option Sstream.matrix =
    let choose (check : TypedExpr.t -> bool) name (hole : hole) (ctx : expr Ctx.t) :
        expr Ctx.t Sstream.matrix =
      Sstream.map_matrix (matrix_of_hole check hole) ~f:(fun x ->
          Map.set ctx ~key:name ~data:x)
    in

    let matrix =
      let named_holes = Map.to_alist spec.Spec.holes in
      ...
      let init_ctx = Map.mapi ~f:(fun ~key ~data:_ -> `Id key) spec.Spec.holes in

      let check' name (e : TypedExpr.t) : bool =
        (* let nesting_depth_cap = 2 in *)
        (* let tctx = Ctx.merge_right init_tctx (free e |> Ctx.of_alist_exn) in *)
        (* let max_depth = max (Map.data tctx |> List.map ~f:type_nesting_depth) in *)
        (* if max_depth > nesting_depth_cap then false else *)
        let ctx = Map.set init_ctx ~key:name ~data:(TypedExpr.to_expr e) in
        ...
        | `Let _ ->
            ...
            List.for_all (List.zip_exn examples result_sterms)
              ~f:(fun ((input, _), result_sterm) ->
                let expr = ExprValue.of_expr ((spec.Spec.target ctx) input) in
                try
                  let lhs : ExprValue.t =
                    run_with_time "partial_eval" (fun () ->
                        Eval.partial_eval ~recursion_limit:100
                        ...
                        try
                          let _ =
                            run_with_time "unify" (fun () ->
                                Unify.unify_one (Unify.translate lhs_term)
      ...
      let check name (e : TypedExpr.t) : bool =
        ...
        run_with_probability ~default:true
          ~f:(fun () ->
            Counter.incr counter "num_checks_run";
            check' name e)
...
let solve ?(config = Config.default) ?(bk = []) ?(init = default_init) examples =
  ...
  let verify ?(limit = 100) (target : _ -> Expr.t) examples =
    ...
    try
      ...
      | `Let (name, body, _) ->
          infer_exn (Map.set tctx ~key:name ~data:(fresh_free 0)) body |> ignore;
          Verify.verify_examples ~limit target examples
      | _ -> failwith "Bad result from solve_single."
    with TypeError _ | Eval.RuntimeError _ -> false
  in
